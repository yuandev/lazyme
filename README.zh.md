...
- **状态页**: 实时展示已部署提交、git 状态、构建/运行命令、JVM 参数、环境变量、PID、运行时长、内存占用
- **侧边栏**: 按分组展示服务卡片，在线/离线状态、内存总计
- **提交页**: 最近提交列表，支持回滚和查看日志
- **历史页**: 部署历史记录，显示缓存状态
- **配置页**: 编辑 target config、Maven 配置、`vite.config.ts`、JVM 参数和环境变量
- **分支切换**: 下拉框展示所有远程分支，切换后自动持久化
- **克隆目标**: 复制目标配置到自定义路径，独立运行，互不影响
- **打开服务**: 一键在浏览器中打开已部署的服务
- **中英文切换**: header 右侧语言切换按钮
- **WebSocket 实时日志**: 流式查看构建输出
...
在 `[run]` 中设置 `mode = "dev"` 跳过构建步骤。发现新提交时，lazyme 只 pull 代码**不重启**进程——HMR（Vite、nodemon 等）会自动处理文件变更。仅在进程崩溃时才自动重启。

### 进程恢复

lazyme 重启后通过多因素校验自动发现已运行的服务：
1. 检查配置的 TCP 端口（`fuser`）
2. 匹配 artifact 关键字（`pgrep -f`）
3. 从 shell 包装进程下钻到真正的 Java PID（`/proc`）

恢复的进程可正常显示内存（VmRSS）、健康状态，不会触发重复部署。
...
前端内置标准的性能监控系统，点击顶部导航栏右侧的 `📊` 按钮打开性能面板：

- **Web Vitals**: LCP / FID / INP / CLS 核心网页指标
- **API 统计**: 每个端点的调用次数、平均耗时、慢请求率、错误率
- **内存监控**: JavaScript 堆内存使用情况与限制
...
性能评级标准：
- 🟢 **良好**: LCP < 2500ms, FID < 100ms, INP < 200ms, CLS < 0.1
- 🟡 **需改进**: LCP < 4000ms, FID < 300ms, INP < 500ms, CLS < 0.25
- 🔴 **较差**: 超过上述阈值
